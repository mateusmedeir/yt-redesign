...
function WatchCommentsReply(comment, toolbar) {
  ...
  expanderButtonText.innerHTML = moreRepliesButton
    ?.querySelector('.yt-spec-button-shape-next__button-text-content span')
    .innerHTML.split(' ')[0]
  ...
  if (moreRepliesButton == null) {
    ...
    expanderButton.addEventListener('click', () => {
      if (!moreRepliesButton.hidden) moreRepliesButton.click()
      else lessRepliesButton.click()
    })
  }

  return replyAndMoreRepliesWrapper
}

function WatchCommentsCallback(commentsWrapper) {
  const comments = commentsWrapper.querySelectorAll(
    'ytd-comment-thread-renderer:not(.yt-watch-comment), #expander-contents ytd-comment-view-model:not(.yt-watch-comment)'
  )
  ...
  comments.forEach(comment => {
    ...
    const replyAndMoreRepliesWrapper = WatchCommentsReply(comment, toolbar)
    if (!replyAndMoreRepliesWrapper) return false

    buttonsWrapper.appendChild(likeDislikeWrapper)
    if (creatorHeart.children.length > 0)
      buttonsWrapper.appendChild(creatorHeart)
...
function WatchComments() {
  ...
  commentsObserver.observe(commentsWrapper, {
    childList: true,
    subtree: true,
    attributes: true
  })
